<x-layout.app>
    ...
    <form action="{{ route('links.create')}}" method="post" enctype="multipart/form-data" class="flex flex-col justify-between pt-15 font-(family-name:--outfit)">
        @csrf
            ...
            <div class="flex flex-col w-3/4 text-sm">
                <div class="flex gap-1">
                    <div>
                        ...
                        <input type="text" placeholder="Digite o nome do conteúdo" name="name" class="mt-2 bg-(--scure) h-10 rounded-xl input input-md focus:outline-0 w-full text-sm">
                    </div>

                    <div>
                        <label for="streaming" class="mb-2">Plataforma de streaming</label>
                        <input type="text" name="streaming" placeholder="Onde você está assistindo?" class="mt-2 bg-(--scure) h-10 rounded-xl input input-md focus:outline-0 w-full">
                    </div>
                </div>

                <div class="mt-4 text-sm">
                    <label for="link" class="">URL</label>
                    <input type="text" name="link" placeholder="Cole a URL do conteúdo" class="mt-2 bg-(--scure) h-10 rounded-xl input input-md focus:outline-0 w-full">
                </div>
